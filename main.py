...
class TicTacToe:
    ...
    def click(self, event):
        ...
        # Computer player's move (randomly choosing an empty cell)
        elif self.current_player == "o":
            print('Computer player\'s move:')
            empty_cells = [(i, j) for i in range(3) for j in range(3) if self.get_cell(i, j) == ""]
            if empty_cells:
                computer_move = random.choice(empty_cells)
                i, j = computer_move
                print(f'Cell {i}, {j} chosen by the computer')
                self.set_cell(i, j, self.current_player)
                self.next_turn()
            else:
                print('No empty cells left, game ended')
                self.current_player = ""  # Game ended
